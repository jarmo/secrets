...
func main() {
  switch parsedCommand := cli.Execute(VERSION, os.Args[1:]).(type) {
    case command.List:
      secrets, _, _ := loadVault(vaultPath(parsedCommand.VaultAlias, parsedCommand.VaultPath))
      for _, secret := range vault.List(secrets, parsedCommand.Filter) {
        fmt.Println(secret)
      }
    case command.Add:
      secrets, path, password := loadVault(vaultPath(parsedCommand.VaultAlias, parsedCommand.VaultPath))
    ...
    case command.ChangePassword:
      ...
      if err := vault.ChangePassword(vaultPath(parsedCommand.VaultAlias, parsedCommand.VaultPath), currentPassword, newPassword, newPasswordConfirmation); err != nil {
        fmt.Println(err)
        os.Exit(1)
      } else {
    ...
    case command.Initialize:
      configurationPath := path.Store(parsedCommand.VaultPath, parsedCommand.VaultAlias)
...
func vaultPath(alias string, customPath string) string {
  if customPath != "" {
    return customPath
  } else {
